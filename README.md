...
A daily long-side stock screener for NYSE + NASDAQ (including ETFs and ADRs).
The system applies a 6-category filter to every closed-candle session and
emits both a per-run CSV (all stocks, passed + rejected) and a Telegram
broadcast of the day's candidates.
...
| Stage | Module |
|---|---|
| 1. Data infrastructure | `stage1_data/` (Phase A universe + SEC EDGAR exchange labels, Phase B Finnhub OHLCV + marketCap with strict validation, Phase C SMA20 / Wilder ATR% / Lambert CCI14 / open gaps) |
