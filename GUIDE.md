...
**קטגוריה 5 — הגאפ הקרוב מכריע:** עד היום הקטגוריה עברה ברגע שהיה גאפ
פתוח כלשהו מעל המחיר, בלי קשר למרחק. גאפ במרחק 15 ATR מלמעלה היה מכשיר
מניה שיש לה גאפ חצי ATR מתחת — והקרוב הוא זה שבאמת מושך את המחיר.

עכשיו כל גאפ נמדד אל **הקצה הקרוב שלו** — הנקודה הראשונה שהמחיר יפגוש —
והקרוב ביותר מכריע:

| מצב | תוצאה |
|---|---|
| המחיר בתוך גאפ | ✅ עוברת (הוא ממלא אותו כרגע, אין מגנט) |
| אין גאפים פתוחים בכלל | ✅ עוברת |
| הגאפ הקרוב **מלמעלה** | ✅ עוברת — יש מקום לרוץ |
| הגאפ הקרוב **מלמטה** | ❌ **נפסלת**, לא משנה מה יש מעליה |

**אין סף מרחק** — גאפ מלמטה פוסל בכל מרחק, בדיוק כמו שהקוד כבר התייחס
לגאפ בודד מלמטה. תיקו מדויק מוכרע לטובת "מלמטה".

המרחק מדווח ביחידות ATR בעמודה `Gap_Nearest_ATR`, אבל הדירוג נעשה על
המרחק הגולמי — כל הגאפים של אותה מניה מתחלקים באותו ATR, אז הסדר זהה,
ואין חשיפה לחלוקה באפס.

שים לב: גאפים מזוהים **רק בתוך חלון 60 הימים**. גאפ שנוצר לפני כן לא
קיים מבחינת הקטגוריה הזאת.

**עמודות CSV חדשות:** `Gap_Nearest`, `Gap_Nearest_ATR`.
