...
1. **Phase A — `/stable/company-screener`**: שתי קריאות (NASDAQ + NYSE).
   ...
   **ETFs, קרנות סגורות ו-ADR זרים כלולים ביקום.** אין סינון לפי
   ...
   `isActivelyTrading=true` כן נשלח, אבל הוא לא קריטריון בחירה: נייר
   מושעה או מחוק ממילא חסר ווליום ויפול על סף המיליון. השארתו רק
   חוסכת קריאת FMP מיותרת לכל טיקר מת.
